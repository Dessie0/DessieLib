 ...
 * recomposing multiple paths from the {@link StorageContainer} back into
 ...
 * completed Object.
 *
 * @see StorageDecomposer to add the Recomposer to the {@link StorageContainer}s usage list.
...
public class RecomposedObject<T> {
    ...
    /**
     * Creates an empty RecomposedObject. Generally should not be used, use {@link RecomposedObject#filled(StorageContainer, Class)} instead.
     * 
     * @param api The IStorageAPI instance.
     */
    public RecomposedObject(IStorageAPI api) {
        this.api = api;
    }
     ...
     *             Generally, you'll want to return {@link StorageContainer#retrieveAsync(String)}.
     ...
     *             Generally, you'll want to return {@link StorageContainer#retrieve(String)}
    ...
    public <M> M getCompletedObject(String path) {
        ...
        M completed = (M) future.join();
        if(completed == null && this.getAPI().getWrappers().containsValue(this.getType(path))) {
            return (M) this.getAPI().getDefault(this.getType(path));
        }
    ...
    /**
     * Returns the {@link IStorageAPI} instance that was used to create this Object.
     * @return The IStorageAPI instance.
     */
    ...
    public static <T> RecomposedObject<T> filled(StorageContainer container, Class<T> classType) {
        ...
        StorageDecomposer<T> decomposer = container.getAPI().getDecomposer(classType);
