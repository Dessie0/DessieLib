...
import me.dessie.dessielib.storageapi.decomposition.RecomposedObject;
...
import me.dessie.dessielib.storageapi.util.Triple;
 ...
 *            Generally, this will be some natively supported array implementation such as {@link ArrayList}.
 */
public abstract class ArrayContainer<H> extends StorageContainer {
     ...
     */
    public ArrayContainer(IStorageAPI api) {super(api);}
     ...
     */
    public ArrayContainer(IStorageAPI api, StorageSettings settings) {
        super(api, settings);
    }
     ...
     *
     * Note that objects will only be removed from the list if {@link Object#equals(Object)} returns true.
     *
    ...
    public <T> List<T> retrieveList(Class<T> type, String path) {
        ...
        list = this.handleRetrieveList(handler, this.getAPI().getDecomposer(type) != null ? type : null);
    ...
    protected Object handleNestedObject(H handler, Class<?> decomposerType) {
        StorageDecomposer<?> decomposer = this.getAPI().getDecomposer(decomposerType);
    ...
    public boolean isListSupported(Class<?> clazz) {
        return clazz != null && clazz.isPrimitive()
                || clazz == String.class
                || this.getAPI().getSupportedPrimitives().contains(clazz);
    }
    ...
    public boolean isPrimitiveList(Object list) {
        ...
        Predicate<Object> predicate = (obj) -> obj.getClass() == String.class || obj.getClass().isPrimitive() || this.getAPI().getWrappers().containsValue(obj.getClass());
     ...
     * e.g. They must be primitives or a have a {@link StorageDecomposer}.
     *
    ...
    protected H handleList(Object data) {
        ...
        recursive.function = (triple) -> {
            ...
            for (String decomposedPath : composer.applyDecompose(triple.getMiddle()).getDecomposedMap().keySet()) {
                ...
                if (storedObject != null && this.getAPI().getDecomposer(storedObject.getClass()) != null) {
                    StorageDecomposer<?> nestedDecomposer = this.getAPI().getDecomposer(storedObject.getClass());
                    currentPath.append(".");
        ...
        this.getListStream(data).forEach(obj -> {
            ...
            StorageDecomposer<?> decomp = this.getAPI().getDecomposer(obj.getClass());
    ...
    /**
     * Inner class used for recursively calling a {@link Function}.
     *
